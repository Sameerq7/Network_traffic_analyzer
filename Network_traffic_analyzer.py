...
def analyze_packet_data(df):
    ...
    percentages = ip_communication_protocols.groupby(["Source IP", "Destination IP"])["Count"].apply(lambda x: x / x.sum() * 100).reset_index(name='Percentage')
    ip_communication_protocols = ip_communication_protocols.merge(percentages, on=["Source IP", "Destination IP"])
...
def plot_share_of_protocols_between_ips(ip_communication_protocols):
    ...
    ip_communication_protocols.groupby('Protocol').sum()['Count'].plot(kind='pie', autopct='%1.1f%%')
...
def main(pcap_file, port_scan_threshold):
    ...
    plot_all_graphs(protocol_counts["Percentage"], ip_communication_protocols)
